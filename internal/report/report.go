// Package report generates various output formats for lineage graphs:
// DOT visualization, JSON reports, and impact summary reports.
package report
...
// WriteDOT writes a Graphviz DOT representation of the graph.
...
// JSONReport is the structure written by WriteJSON.
...
// JSONNode describes a node in the report.
...
// JSONEdge describes an edge in the report.
...
// JSONValidation summarizes validation results.
...
// WriteJSON writes a comprehensive JSON report of the graph.
...
// ImpactSummary describes the downstream impact of changes to specified nodes.
...
// WriteImpactSummary computes and writes a JSON impact report for the given changed nodes.
...
// ComputeImpactSummary builds the impact summary without writing it.
func ComputeImpactSummary(g *graph.Graph, changed []string) (*ImpactSummary, error) {
	...
	// Compute max distance from any changed node to any impacted node
	maxDist := 0
	...
	// Find critical paths from changed to leaves among impacted
	leaves := g.Leaves()
...
// WriteTextSummary writes a human-readable text summary of the graph.
...
// DepMatrix holds a node-to-node dependency matrix (direct dependencies only).
type DepMatrix struct {
	Nodes  []string `json:"nodes"`
	Matrix [][]bool `json:"matrix"`
}

// BuildDepMatrix constructs a direct dependency matrix.
...
// WriteDepMatrix writes the dependency matrix as JSON.
...
// ComputeStats calculates various graph statistics.
...
// WriteStats writes statistics as JSON.
...
// OwnerReport groups nodes by owner.
type OwnerReport struct {
	Owners map[string][]string `json:"owners"`
}

// BuildOwnerReport creates an owner-grouped report.
