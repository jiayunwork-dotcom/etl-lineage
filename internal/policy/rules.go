...
// LayerFlowRule enforces that edges only flow from lower layers to higher layers.
// Layer hierarchy: staging < dim < fact < report.
...
// OwnerBoundaryRule limits the number of cross-team dependencies.
...
// TransformAllowlistRule ensures all edge transforms are in an allowed set.
...
// MaxFanInRule flags nodes with too many incoming edges.
...
// MaxFanOutRule flags nodes with too many outgoing edges.
...
// NoSelfOwnerRule flags nodes that depend on themselves (via intermediary with same owner).
...
func (r *NoSelfOwnerRule) Evaluate(g *graph.Graph) []Violation {
	// Self-loops are already prevented by graph.AddEdge, so this is a no-op
	// but validates the invariant holds post-deserialization.
	var viols []Violation
	for _, e := range g.Edges() {
		if e.From == e.To {
			viols = append(viols, Violation{
