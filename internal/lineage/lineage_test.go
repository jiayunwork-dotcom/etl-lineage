...
// build creates: stg_orders -> dim_orders -> fact_sales -> report_kpi
//                stg_customers -> dim_customers -> fact_sales
...
func TestBatchImpactMultipleChanges(t *testing.T) {
	...
	// Both paths converge at fact_sales -> report_kpi, plus dim_orders and dim_customers
...
func TestAllPathsMultiplePaths(t *testing.T) {
	g := buildPipeline()
	// stg_orders -> dim_orders -> fact_sales -> report_kpi (length 4)
	// There's only one path from stg_orders to report_kpi
...
func TestCriticalPath(t *testing.T) {
	...
	// longest chain: stg_orders->dim_orders->fact_sales->report_kpi (4 nodes)
	if len(cp) < 4 {
...
func TestCommonAncestors(t *testing.T) {
	g := buildPipeline()
	// Common ancestors of dim_orders and dim_customers: none (they don't share upstream)
