// Package metrics computes structural health metrics for lineage graphs.
// These quantitative measures help identify complexity hotspots, coupling,
// and pipeline fragility.
package metrics
...
// GraphMetrics holds computed metrics for a lineage graph.
type GraphMetrics struct {
	...
	Depth           int     `json:"depth"`            // longest path in DAG
	Width           int     `json:"width"`            // max nodes at any single level
	RootCount       int     `json:"root_count"`
	LeafCount       int     `json:"leaf_count"`
	Components      int     `json:"components"`       // weakly connected
	AvgPathLength   float64 `json:"avg_path_length"`
	CouplingScore   float64 `json:"coupling_score"`   // cross-owner edge ratio
}

// Compute calculates all metrics for the given graph.
...
// Summary returns a formatted text summary of the metrics.
...
// ComplexityScore returns a single composite score (0-100) indicating graph complexity.
// Higher = more complex. Factors: density, depth, max fan-in, coupling.
...
// Hotspots returns nodes sorted by a combined "risk" score (high fan-in + fan-out).
