...
func TestComputeDepth(t *testing.T) {
	m := Compute(pipelineGraph())
	// longest path: src->dim->fact->report = 3 edges
	if m.Depth != 3 {
...
func TestComputeMaxFanIn(t *testing.T) {
	m := Compute(pipelineGraph())
	// dim has fan-in 2 (from src1, src2)
	if m.MaxFanIn != 2 {
...
func TestComputeCoupling(t *testing.T) {
	m := Compute(pipelineGraph())
	// cross-owner edges: src1->dim(a->b), src2->dim(a->b), fact->report(b->c) = 3 of 4
...
func TestHotspots(t *testing.T) {
	...
	// dim has fan-in 2 + fan-out 1 = 3, fact has 1+1=2
	if spots[0] != "dim" {
...
func TestCheckHealthCritical(t *testing.T) {
	// create deep graph to exceed depth limit
	g := graph.New()
	prev := "n0"
	g.AddNode(prev)
	for i := 1; i <= 15; i++ {
